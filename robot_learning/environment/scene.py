...
from robot_learning.task import BinSortTaskSpec, HW1_TASK
from util.paths import REPO_ROOT

TASK_ASSETS_DIR = REPO_ROOT / "robot_learning" / "assets"
ROBOT_ASSETS_DIR = REPO_ROOT / "mujoco_irb120" / "robot" / "assets"
GENERATED_SCENE_PATH = Path(gettempdir()) / "mujoco_irb120_hw1_binsort.xml"


def create_hw1_scene_xml(
    task: BinSortTaskSpec = HW1_TASK,
    template_path: Path = TASK_ASSETS_DIR / "scene_template.xml",
    ...
    with open(out, "w", encoding="utf-8") as f:
        f.write(
            template.format(
                robot_mesh_dir=(ROBOT_ASSETS_DIR / "robot" / "visual").as_posix(),
...
def _hw1_world_block(task: BinSortTaskSpec) -> str:
    ...
    robot_xml = (ROBOT_ASSETS_DIR / "robot" / "robot.xml").as_posix()
    sort_cube_xml = (TASK_ASSETS_DIR / "sort_cube.xml").as_posix()
